...
export default function OrderCard({ order, onContinue, onCancel }: Props) {
   ...
   return (
      <div className="bg-(--surface-2) border border-(--line) rounded-(--r-md) p-3 space-y-2.5 dark:bg-zinc-100 dark:border-zinc-300">
         <div className="flex items-start justify-between gap-2">
            <p className="sans text-xs text-(--text-faint) dark:text-(--surface)">Pedido #{order.id}</p>
